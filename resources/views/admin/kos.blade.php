@extends('layouts.app')
@section('title', 'Kelola Kos – Admin BaKos')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="font-display text-2xl font-bold text-gray-900">Kelola Kos</h1>
            <p class="text-gray-400 text-sm">Total {{ $kosList->total() }} kos terdaftar</p>
        </div>
    </div>

    {{-- Search --}}
    <form method="GET" action="{{ route('admin.kos') }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5 flex gap-3">
        <div class="relative flex-1">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama kos..."
        ...
        <a href="{{ route('kos.create') }}" class="flex items-center gap-2 border border-brand-200 text-brand-600 hover:bg-blue-50 font-medium px-5 py-2.5 rounded-xl text-sm transition">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">
                        ...
                        <th class="px-6 py-3 hidden md:table-cell">Pemilik</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900">{{ Str::limit($kos->nama, 28) }}</p>
                                    <p class="text-xs text-gray-400">{{ $kos->kecamatan }}, {{ $kos->kota }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.kos.delete', $kos) }}"
                                      onsubmit="return confirm('Hapus kos {{ $kos->nama }}?')">
                                    @csrf @method('DELETE')
