...
<div class="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-3xl shadow-float border border-slate-100 p-8 sm:p-10">
            ...
            @if($errors->any())
            <div class="bg-red-50 border border-red-100 text-red-600 rounded-xl px-4 py-3 text-sm mb-6 flex items-start gap-3">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus
                           ...
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:bg-white focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all @error('email') border-red-300 focus:border-red-500 focus:ring-red-500/10 @enderror">
                </div>
                ...
                <div class="flex items-center pt-2">
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-4 h-4 text-brand-600 border-slate-300 rounded focus:ring-brand-500 cursor-pointer">
                        <span class="text-sm text-slate-600 group-hover:text-slate-900 transition-colors">Ingat saya</span>
                    </label>
                </div>

                <button type="submit"
                        class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl text-sm transition-all shadow-md mt-4">
                    Masuk ke Akun
                </button>
            </form>
        </div>
